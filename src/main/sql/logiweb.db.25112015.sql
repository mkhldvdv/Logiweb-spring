...
-- Created by mkhldvdv on 25.11.2015, 0:10:01 --
------------------------------------------------

spool logiweb.db.25112015.log
...
prompt
prompt Creating table USER_STATUSES
prompt ============================
prompt
create table logiweb.USER_STATUSES
(
  user_status_id   NUMBER not null,
  user_status_name VARCHAR2(30) not null
)
;
alter table logiweb.USER_STATUSES
...
alter table logiweb.USERS
  ...
  references logiweb.USER_STATUSES (USER_STATUS_ID);
create index logiweb.USER_CITY_IDX on logiweb.USERS (CITY_ID);
create index logiweb.USER_ROLE_IDX on logiweb.USERS (ROLE_ID);
create index logiweb.USER_TRUCK_IDX on logiweb.USERS (TRUCK_ID);
create index logiweb.USER_USER_STATUS_IDX on logiweb.USERS (USER_STATUS_ID);
...
create index logiweb.ORDER_USER_IDX on logiweb.ORDER_DRIVER (USER_ID);
